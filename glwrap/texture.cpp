...
void Texture::ReadPPM(const char *filename)
{
	...
	data = (uint8_t*) calloc(width * height, bpp / 8);
	uint8_t *d = data;
...
void Texture::ReadBMP(const char *filename)
{
	...
	data = (uint8_t*) calloc(surface->w * surface->h, 3);
	width  = surface->w;
	height = surface->h;
	format = GL_BGR;
	uint8_t *pixel_data      = (uint8_t*) surface->pixels;
	uint32_t bytes_per_pixel = 3;
	uint32_t bytes_per_row   = 3 * width;
	for(int32_t h = 0; h < surface->h; h++)
	{
		memcpy( data       + h * bytes_per_row,
				pixel_data + h * surface->pitch,
				bytes_per_row);
	}
}
